-- 博客网站初始数据插入脚本
-- 与 create_tables.sql 保持一致
-- 版本: 1.0
-- 更新日期: 2026-01-21

USE blog_db;

-- 禁用外键检查，避免插入数据时因外键约束失败
...
-- 技术分享的子分类（parent_id=1 表示父分类是"技术分享"）
INSERT INTO `categories` (`name`, `description`, `parent_id`, `sort_order`, `article_count`, `status`) VALUES
...
SELECT '用户数据:' as '';
SELECT id, username, email, nickname, role, status FROM users;

SELECT '' as '';
SELECT '分类数据:' as '';
SELECT id, name, parent_id, description, article_count, status FROM categories ORDER BY parent_id, sort_order;

SELECT '' as '';
SELECT '文章数据:' as '';
SELECT id, title, LEFT(summary, 50) as summary, status, view_count, comment_count FROM articles;

SELECT '' as '';
SELECT '评论数据:' as '';
SELECT id, article_id, LEFT(content, 30) as content, status FROM comments;

SELECT '' as '';
SELECT '系统配置:' as '';
SELECT config_key, config_value, description FROM system_config WHERE is_public = 1;
...
SELECT '演示用户账号: demo_user / admin123' as '';
SELECT '========================================' as '';
